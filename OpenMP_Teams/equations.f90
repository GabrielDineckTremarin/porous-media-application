...
subroutine RESU(um,vm,p,RU)
    ...
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO PRIVATE(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dudxdx,dxdvdy,q_art,u_W,u_WW,u_E,u_EE,u_N,u_NN,u_S,u_SS,v_P,u_P,ap,aw,aww,ae,aee,an,ann,as,ass,afw,afe,afn,afs) num_teams(numThreads) thread_limit(32)
    DO j=3,jmax-2
      DO i=3,imax-1
...
subroutine RESV(um,vm,p,T,InvFr2,RV)
    ...
    ! $omp parallel do 
    !$OMP TARGET TEAMS DISTRIBUTE PARALLEL DO private(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dvdydy,dydudx,q_art,v_W,v_WW,v_E,v_EE,v_N,v_NN,v_S,v_SS,v_P,u_P,ap,aw,aww,ae,aee,an,ann,as,ass,afw,afe,afn,afs) num_teams(numThreads) thread_limit(32) 
    DO j=3,jmax-1
      DO i=3,imax-2

...
subroutine RESU(um,vm,p,RU)
    ...
    !$omp parallel do private(i,j,fn,afn,fs,afs,fe,afe,fw,afw,df,Dn,Ds,De,Dw,dudxdx,dxdvdy,q_art,&
    !$omp& u_W,u_WW,u_E,u_EE,u_N,u_NN,u_S,u_SS,u_P,v_P,&
    ...
    !DO i=2,imax
      CALL upwind_U(um,vm,p,RU,2,imax,2,2) 
    !ENDDO
    ! $omp end parallel do

    ! $omp parallel do private(i)
    !DO i=2,imax
      CALL upwind_U(um,vm,p,RU,2,imax,jmax-1,jmax-1)
    !ENDDO
    ! $omp end parallel do
...
!#######################################################################################
!#######################################################################################
subroutine upwind_U(um,vm,p,RU,iii,ii,jjj,jj)!ii e jj são os limites da borda para que o laco passe para dentro de upwind_U
    use comum
    implicit none
    ...
    !$omp parallel do private(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dudxdx,dxdvdy,q_art,&
    !$omp& u_W,u_E,u_N,u_S,u_P,V_P,ap,aw,ae,an,as)
    DO j=jjj,jj
        DO i=iii,ii
...
subroutine RESV(um,vm,p,T,InvFr2,RV)
    ...
    !$omp parallel do private(i,j,fn,afn,fs,afs,fe,afe,fw,afw,df,Dn,Ds,De,Dw,dvdydy,dydudx,q_art,&
    ...
    ! $omp parallel do private (i)
    !DO i=2,imax-1
        CALL upwind_V(um,vm,p,RV,InvFr2,T,2,imax-1,2,2)
    !ENDDO
    ! $omp end parallel do
    
    ! $omp parallel do private (i)
    !DO i=2,imax-1
        CALL upwind_V(um,vm,p,RV,InvFr2,T,2,imax-1,jmax,jmax)
    !ENDDO
    ! $omp end parallel do

    ! $omp parallel do private (j)
    !DO j=2,jmax
        CALL upwind_V(um,vm,p,RV,InvFr2,T,2,2,j,jmax)
    !ENDDO
    ! $omp end parallel do
    
    ! $omp parallel do private (j)
    !DO j=2,jmax
        CALL upwind_V(um,vm,p,RV,InvFr2,T,imax-1,imax-1,j,jmax)
    !ENDDO
    ! $omp end parallel do
...
!#######################################################################################
!#######################################################################################
subroutine upwind_V(um,vm,p,RV,InvFr2,T,iii,ii,jjj,jj)
    use comum
    implicit none
    integer :: i, ii, iii, j, jj, jjj, itc
    ...
    !$omp& v_W,v_E,v_N,v_S,v_P,u_P,ap,aw,ae,an,as)
    DO j=jjj,jj
        DO i=iii,ii
...
SUBROUTINE solve_P(c2,p,um_n,vm_n,pn,residual_p)
    ...
    max_vel = MAXVAL(um_n**2.d0)
    max_vel = max_vel + MAXVAL(vm_n**2.d0)
...
SUBROUTINE RESZ(um_n,vm_n,Z,RZ)
    ...
    !$omp parallel do private(i,j, dZudx,dZvdy, De,Dw,Dn,Ds,Dp, difx,dify, difx2,dify2)   
    do j=2,jmax-1
        do i=2,imax-1
            ...
            RZ(i,j) = 1.d0 / (difx / dify) *&
                    (-Dp*Z(i,j) + De*Z(i+1,j) + Dw*Z(i-1,j) + Dn*Z(i,j+1) + Ds*Z(i,j-1) -&
                    (1.d0-liga_poros(i,j))* (dZudx + dZvdy) )!/epsilon1(i,j))  

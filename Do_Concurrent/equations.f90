...
!#######################################################################################
!#######################################################################################
subroutine upwind_U(um,vm,p,RU,ii,iii,jj,jjj)
    use comum
    implicit none
    integer :: i, j, itc    
...
!ENDDO
    ...
    ! $omp parallel do private (j)
    !DO j=2,jmax
      CALL upwind_V(um,vm,p,RV,InvFr2,T,2,2,2,jmax)
    !ENDDO
    ! $omp end parallel do
    ! $omp parallel do private (j)
    !DO j=2,jmax
      CALL upwind_V(um,vm,p,RV,InvFr2,T,imax-1,imax-1,2,jmax)
    !ENDDO
    ! $omp end parallel do
...
!#######################################################################################
!#######################################################################################
subroutine upwind_V(um,vm,p,RV,InvFr2,T,ii,iii,jj,jjj)
    use comum
    implicit none
    integer :: i, j, itc
    integer :: ii, iii, jj, jjj
    ...
    ! $omp parallel do private(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dvdydy,dydudx,q_art,v_W,v_E,v_N,v_S,v_P,u_P,&
    ! $omp &ap,aw,aww,ae,aee,an,ann,as,ass)
!do j=jj,jjj
    DO CONCURRENT(j=jj:jjj)
  do i=ii,iii
...
!                  - Z(i,j-1) * vm_n(i,j  ) * areav_s(i)
        ...
        De = (ym(j+1)-ym(j)) * (1.d0/Pe)
        Dw = De / (x(i  )-x(i-1)) 
        De = De / (x(i+1)-x(i  ))  
        Dn = (xm(i+1)-xm(i)) * (1.d0/Pe)
        Ds = Dn / (y(j  )-y(j-1))  
        Dn = Dn / (y(j+1)-y(j  )) 

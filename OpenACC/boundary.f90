...
END SUBROUTINE bcZ2
!----------------------------------------------------------------
SUBROUTINE bcZ1(T_n_tau)
    USE comum
    use openacc
    ! $acc routine
    IMPLICIT NONE
    INTEGER :: i, j
    REAL(8), DIMENSION(1:imax,1:jmax) :: T_n_tau!,gradZ_x,gradZ_y
    ! $acc data present (T_n_tau,x, y,dx,dy)
...
!--------contorno inferior e superior --------
    ! $acc parallel loop
    do i=1,imax
        T_n_tau(i,1  ) = T_n_tau(i,2)
        T_n_tau(i,jmax) = T_n_tau(i,jmax-1) + 1.d0*(T_n_tau(i,jmax-1)-T_n_tau(i,jmax-2))
    enddo
    ! $acc end parallel loop
!-------contorno esquerdo e direito -------
    ! $acc parallel loop
    do j=1,jmax
        T_n_tau(1  ,j) = T_n_tau(2,j)
        T_n_tau(imax,j) = T_n_tau(imax-1,j)
    enddo
    ! $acc end parallel loop
    !   i=1
        ! $acc parallel loop

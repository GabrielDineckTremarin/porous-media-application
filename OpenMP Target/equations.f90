...
!$OMP DISTRIBUTE PARALLEL DO collapse(2) private(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dudxdx,dxdvdy,q_art,u_W,u_WW,u_E,u_EE,u_N,u_NN,u_S,u_SS,v_P,u_P&
!$OMP&,ap,aw,aww,ae,aee,an,ann,as,ass, afw, afe, afn, afs) 
DO i=3,imax-1
    DO j=3,jmax-2
...
!    REAL(8), DIMENSION(1:imax+1,1:jmax+1) ::epsilon1,Darcy
    ...
    !$OMP TARGET TEAMS MAP(tofrom: um_tau, res_u, i, j, um, um_tau, RU, res_u, ui, dtau) 
    ...
    !!!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom:I, J, UM, UM_TAU, ru, RES_U, ui, res_u, dtau) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE(i,j) 
    DO i=3,imax-1
        DO j=2,jmax-1
    ...
    !!!!! $omp parallel do
    !$OMP TARGET TEAMS MAP(tofrom: i, j, um,um_tau,RU,res_u, ui, dtau) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=3,imax-1
        DO j=2,jmax-1
...
!$OMP DISTRIBUTE PARALLEL DO collapse(2) private(i,j,fn,fs,fe,fw,df,Dn,Ds,De,Dw,dvdydy,dydudx,q_art,&
!$omp&v_W,v_WW,v_E,v_EE,v_N,v_NN,v_S,v_SS,v_P,u_P,ap,aw,aww,ae,aee,an,ann,as,ass)
DO i=3,imax-2
    DO j=3,jmax-1
...
!    REAL(8), DIMENSION(1:imax+1  ,1:jmax+1) :: epsilon1,Darcy
    ...
    !!!!!  $omp parallel do private(i,j) 
    !$OMP TARGET TEAMS MAP(tofrom: i,j,vm,vm_tau,RV,res_v, vi, dtau) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE(i, j) 
    DO i=2,imax-1
        DO j=3,jmax-1
    ...
    !!!!! $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom:i,j,vm,vm_tau,RV,res_v, vm_n_tau, vi, dtau) 
    !$OMP DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=2,imax-1
        DO j=3,jmax-1
            !call solve_res_v(i,j,vm,vm_tau,RV,res_v)
...
!!! RALSTON'S METHOD (Second Order Runge-Kutta)
    ...
    !!! !  $omp parallel do 
    !$OMP TARGET TEAMS MAP(tofrom: i,j,Z,Z_tau,RZ,res_Z, zi, dtau)
    !$OMP  DISTRIBUTE PARALLEL DO private(i,j) 
    DO i=2,imax-1
        DO j=2,jmax-1
...
!!!!! $omp parallel do private(i,j)
    !$OMP TARGET TEAMS MAP(tofrom: i,j,Z,Z_tau,RZ,res_Z, Z_N_TAU, zi, dtau) 
    !$OMP DISTRIBUTE PARALLEL DO PRIVATE(i, j)  
    DO i=2,imax-1
        DO j=2,jmax-1
